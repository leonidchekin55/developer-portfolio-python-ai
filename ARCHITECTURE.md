...
This profile validates the request flow and UI, but it is not a durable multi-user SaaS deployment. The public Knowledge Assistant runs read-only: it exposes the built-in guide, disables uploads and chat-history persistence, and scopes retrieval to that guide. This avoids exposing visitor uploads through the public demo, but it is not a substitute for tenant isolation in a production service. The private Compose profile can enable uploads with `ALLOW_PUBLIC_UPLOADS=true`; use that only on a trusted instance because it does not add accounts or per-user access control. Data can reset and in-memory Pulseboard events are not durable. The full Compose profiles demonstrate the richer local integrations.
