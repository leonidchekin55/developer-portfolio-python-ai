...
Free Blueprint не создаёт managed PostgreSQL, Redis или persistent disks. Pulseboard использует демонстрационный режим с событиями в памяти. Knowledge Assistant поддерживает внешний PostgreSQL через `DATABASE_URL`, но сохранность данных в облачной конфигурации ещё не подтверждена. Считайте состояние аккаунтов и истории временным, пока новый деплой не станет Live и `/health/ready` не подтвердит соединение. Исходные файлы удаляются после индексации; извлечённые тексты остаются в базе.
