...
Оба проекта включают `.env.example`, демо-данные, health checks и тесты; общий GitHub Actions workflow в корне проверяет Python API и сборку интерфейса. См. README каждого проекта для архитектуры, примеров API и ограничений демо-режима.
...
Публикация изменений в `main` запускает CI и автоматический deploy Render. Репозиторий приватный: для внешнего портфолио можно открыть доступ работодателям или позже сделать репозиторий публичным.
